...
import org.liufeng.course.pojo.Media;
...
public class AdvancedUtil {
	...
	public static String getMediaList(String accessToken) {
		// 拼接请求地址
		String requestUrl = "https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token=ACCESS_TOKEN&type=image&offset=0&count=20";
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken);
		
		
		String jsonData = "{\"type\":\"%s\",\"offset\":%d,\"count\":%d}";
		
		System.out.println("input parameter is:"+String.format(jsonData, "image",0,20));
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", String.format(jsonData, "image",0,20));
		// 如果请求成功
		if (null != jsonObject) {
			try {
				
				System.out.println(jsonObject.getInt("total_count"));
				System.out.println(jsonObject.getInt("item_count"));
				System.out.println(jsonObject.getInt("item_count"));
				System.out.println(jsonObject.getString("item")); 
				//jsonObject.getJSONArray("item");
				//List list=JSONArray.toList(jsonObject.getJSONArray("item"),List.class);
			    
				JSONArray jsonarry=jsonObject.getJSONArray("item");
				List<Media> list=(List<Media>)JSONArray.toCollection(jsonarry, Media.class);
			    for (Media tt : list) {
			    	System.out.println("media_id is:"+tt.getMedia_id());
			    	System.out.println("name is:"+tt.getName());
			    	System.out.println("update time is:"+formatTime(tt.getUpdate_time()));
			    	System.out.println("url is:"+tt.getUrl());
			    }  
			} catch (JSONException e) {
				
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				log.error("获取素材 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return "";
	}
